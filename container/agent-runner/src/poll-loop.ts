  ...
  const pollHandle = setInterval(() => {
    ...
    void (async () => {
      try {
        ...
        // Accumulate gate: same as the cold-batch path above. Without this, a
        // trigger=0 (context-only) message landing during an active query would
        // push as a follow-up and engage the agent — the exact opposite of the
        // accumulate-only contract.
        ...
        const formattedFollow = formatMessagesForPrompt(keep);
        log(
