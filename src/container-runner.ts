...
export async function buildMounts(
  ...
  // FORK: SSH keys — per-group opt-in via SSH_ALLOWED_FOLDERS env (comma-
  // separated group folders). v1's run_ssh used a host IPC proxy; v2 has no
  // IPC, so the host SSH dir is bind-mounted read-only into authorized agent
  // group containers instead. Keys are never copied. Classed
  // 'allowlisted-extra' (operator-vetted, read-only): it lives under $HOME,
  // which no pinned mount root claims, so admission judges it by that class.
  ...
  if (sshAllowlist.includes(agentGroup.folder)) {
    ...
    if (process.env.HOME && fs.existsSync(hostSshDir)) {
      mounts.push({
        hostPath: hostSshDir,
