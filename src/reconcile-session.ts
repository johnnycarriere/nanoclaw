...
import { openInboundDb, openOutboundDb } from './mailbox/sqlite/session-db.js';
...
async function reconcileActiveSession(session: Session): Promise<void> {
  ...
  try {
    ...
    // FORK-HOOK:status-reactions+transient-retry:start
    // Runs AFTER the mailbox session above has closed — never inside it: the
    // hooks open their own short-lived raw SQLite handles (one writer per
    // file; a nested same-key session would deadlock).
    await runForkSessionHooks(agentGroup.id, session);
    // FORK-HOOK:status-reactions+transient-retry:end
...
/**
 * FORK: host-side status emoji (👨‍💻 on claim, 👍 on done) and the
 * transient-API-error re-queue. Both read the session mailboxes through raw
 * SQLite handles (this install composes the SQLite mailbox), opened briefly
 * per tick and closed in `finally`. Errors are isolated per session so one
 * broken file never skips the rest of the sweep.
 */
async function runForkSessionHooks(agentGroupId: string, session: Session): Promise<void> {
  ...
  let inDb: Database.Database | undefined;
