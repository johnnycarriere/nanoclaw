...
CREATE TABLE IF NOT EXISTS web_mcp_oauth_clients (
  ...
  response_types_json    TEXT,
  scope                  TEXT
);

CREATE TABLE IF NOT EXISTS web_mcp_oauth_codes (
...
CREATE INDEX IF NOT EXISTS idx_web_mcp_oauth_codes_created ON web_mcp_oauth_codes(created_at_ms);
`;
...
/** @internal shared DB handle for MCP OAuth persistence */
export function getAuthDbInternal(): Database.Database {
  return getAuthDb();
}
